...
import { ArrowUpRight, ShieldCheck, Activity, CheckCircle2, AlertCircle, RotateCcw, Clock, ScrollText, Link as LinkIcon, Zap } from 'lucide-react'
...
export function TrancheBlock({ tranche }: TrancheBlockProps) {
  ...
  const getStatusConfig = () => {
    switch (tranche.status) {
      ...
      case 'EXECUTING': 
        return { color: 'text-primary', icon: Zap, label: 'Executing', rail: 'bg-primary animate-flow-trace', glow: 'shadow-primary/30', isPulse: true }
      case 'FAILED': 
        return { color: 'text-destructive', icon: AlertCircle, label: 'Failed', rail: 'bg-destructive', glow: '', isPulse: false }
      case 'REFUNDED': 
        return { color: 'text-muted-foreground', icon: RotateCcw, label: 'Refunded', rail: 'bg-border', glow: '', isPulse: false }
      default: 
        return { color: 'text-secondary', icon: Clock, label: 'Dormant', rail: 'bg-secondary', glow: 'shadow-secondary/20', isPulse: false }
    }
  }
        ...
        <div className="pl-14 pr-10 py-12 flex flex-col min-h-[420px] justify-between relative z-10">
          ...
          <div className="flex justify-between items-start mb-10">
            <div className="flex flex-col gap-3">
              <div className="px-3 py-1 bg-black/5 border border-black/5 flex items-center gap-3">
                ...
                <span className="text-[11px] font-mono font-bold text-black/60 tracking-widest uppercase">#{tranche.id}</span>
              </div>
          ...
          <div className="space-y-4 mt-auto">
            <div className="bg-black/5 border border-black/5 p-6 flex items-center justify-between border-l-2 border-primary/20">
              ...
              <span className="text-2xl font-mono font-bold tracking-tighter italic text-black">{tranche.capital}</span>
            </div>
            <div className="bg-black/5 border border-black/5 p-6 flex items-center justify-between border-l-2 border-black/10">
              ...
              <span className="text-2xl font-mono font-bold tracking-tighter italic text-black">{tranche.agents} / 25</span>
            </div>
          </div>
